...
class PianoteqHandler(ZynthianBasicHandler):
    ...
    def get(self, errors=None):
        ...
        config['ZYNTHIAN_PIANOTEQ_VERSION'] = ".".join(
            str(v) for v in PIANOTEQ_VERSION)
    ...
    def do_install_pianoteq_binary(self, filename):
        ...
        command = self.recipes_dir + \
            "/install_pianoteq_binary.sh {}".format(filename)
    ...
    def do_activate_license(self):
        ...
        if result != "Activation Key Saved !\n":
            logging.error(
                "Pianoteq License Activation Failed: {}".format(result))
            return result
