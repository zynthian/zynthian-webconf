...
class PresetsConfigHandler(ZynthianBasicHandler):
    ...
    def get(self):
        ...
        config['musical_artifact_tags'] = self.get_argument(
            'MUSICAL_ARTIFACT_TAGS', '')
    ...
    def post(self, action):
        try:
            ...
            if self.engine_cls == zynthian_engine_jalv:
                self.engine_cls.init_zynapi_instance(
                    self.engine_info[0], self.engine_info[2])

        except Exception as e:
            logging.error(
                "Can't initialize engine '{}': {}".format(self.engine, e))
