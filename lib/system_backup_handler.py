...
class SystemBackupHandler(ZynthianBasicHandler):
    ...
    def do_get(self, active_tab, errors=None):
        ...
        system_backup_items = get_backup_items(
        ...
        for system_backup_folder in system_backup_items:
            if system_backup_folder.startswith("^"):
                config['ZYNTHIAN_SYSTEM_BACKUP_DIRECTORIES_EXCLUSIONS'].append(
                    system_backup_folder[1:])
            else:
                config['ZYNTHIAN_SYSTEM_BACKUP_DIRECTORIES'].append(
                    system_backup_folder)

        data_backup_items = get_backup_items(
            SystemBackupHandler.DATA_BACKUP_ITEMS_FILE)
        for data_backup_folder in data_backup_items:
            if data_backup_folder.startswith("^"):
                config['ZYNTHIAN_DATA_BACKUP_DIRECTORIES_EXCLUSIONS'].append(
                    data_backup_folder[1:])
            else:
                config['ZYNTHIAN_DATA_BACKUP_DIRECTORIES'].append(
                    data_backup_folder)
        ...
        def add_data_backup_item(dirname, subdirs, files):
            add_backup_item(dirname, subdirs, files,
                            'ZYNTHIAN_DATA_BACKUP_ITEMS')

        def add_system_backup_item(dirname, subdirs, files):
            add_backup_item(dirname, subdirs, files,
                            'ZYNTHIAN_SYSTEM_BACKUP_ITEMS')
    ...
    def do_system_backup(self):
        self.do_backup('zynthian_system_backup',
                       SystemBackupHandler.SYSTEM_BACKUP_ITEMS_FILE)

    def do_data_backup(self):
        self.do_backup('zynthian_data_backup',
                       SystemBackupHandler.DATA_BACKUP_ITEMS_FILE)

    def do_backup(self, backupFileNamePrefix, backupItemsFileName):
        zipname = '{0}{1}.zip'.format(
            backupFileNamePrefix, time.strftime("%Y%m%d-%H%M%S"))
        f = BytesIO()
            ...
            if sourceFolder.startswith("^"):
                ...
                exclude_filename = '/' + \
                    os.path.basename(os.path.normpath(
                        sourceFolder)) + SystemBackupHandler.EXCLUDE_SUFFIX
                with open(exclude_filename, "w") as exclude_file:
                    for dirname, subdirs, files in os.walk(sourceFolder):
                        for filename in files:
                            exclude_file.write(os.path.join(
                                dirname, filename) + '\n')
                logging.info(exclude_filename)
                # convert single string to array of 1 string
                worker('/', None, exclude_filename[1:].split(':'))
...
class RestoreMessageHandler(ZynthianWebSocketMessageHandler):
    ...
    def on_websocket_message(self, restoreFile):
        ...
        with open(restoreFile, "rb") as f:
            validRestoreItems = get_backup_items(
                SystemBackupHandler.SYSTEM_BACKUP_ITEMS_FILE)
            validRestoreItems += get_backup_items(
                SystemBackupHandler.DATA_BACKUP_ITEMS_FILE)
                ...
                for member in restoreZip.namelist():
                    if self.is_valid_restore_item(validRestoreItems, member):
                        ...
                        message = ZynthianWebSocketMessage(
                            'RestoreMessageHandler', logMessage)
                        ...
                        if member.endswith(SystemBackupHandler.EXCLUDE_SUFFIX):
                            ...
                            with open("/" + member, 'r') as exclude_file:
                                message = ZynthianWebSocketMessage(
                                    'RestoreMessageHandler', "<b>PLEASE ENSURE THAT THE FOLLOWING FILES EXIST:<br />" + exclude_file.read().replace('\n', '<br />') + "</b>")
                                self.websocket.write_message(
                                    jsonpickle.encode(message))
                            os.remove('/' + member)
                        else:
                            logging.warn("restore of " +
