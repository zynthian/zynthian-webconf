        ...
        try:
            for interface_byte in check_output("ifconfig -a | sed 's/[ \t].*//;/^$/d'", shell=True).splitlines():
                ...
                if interface.startswith("wlan"):
                    ...
                    logging.info(
                        "Scanning wifi networks on {}...".format(interface))
                    ...
                    for line_byte in check_output("iwlist {0} scan | grep -e ESSID -e Encryption -e Quality".format(interface), shell=True).splitlines():
                        ...
                        if line.find('ESSID') >= 0:
                            network = {'encryption': False,
                                       'quality': 0, 'signalLevel': 0}
                            ssid = line.split(':')[1].replace("\"", "")
                            if ssid:
                                self.add_network(
                                    wifiList, ssid, network, encryption, quality, signal_level)
                            ...
                            m = re.match(
                                '.*Quality=(.*?)/(.*?) Signal level=(.*?(100|dBm)).*', line, re.M | re.I)
                            if m:
                                quality = round(
                                    int(m.group(1)) / int(m.group(2)) * 100, 2)
                                signal_level = m.group(3)

            wifiList = OrderedDict(
                sorted(wifiList.items(), key=lambda x: x[1]['quality']))
