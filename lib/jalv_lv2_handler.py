...
class JalvLv2Handler(ZynthianBasicHandler):
    ...
    def get(self, errors=None):
        ...
        if not 'ZYNTHIAN_ACTIVE_TAB' in config or len(config['ZYNTHIAN_ACTIVE_TAB']) == 0:
            config['ZYNTHIAN_ACTIVE_TAB'] = zynthian_lv2.PluginType.MIDI_SYNTH.value.replace(
                " ", "_")

        try:
            config['ZYNTHIAN_JALV_FILTER'] = self.get_argument(
                'ZYNTHIAN_JALV_FILTER')
        except:
            config['ZYNTHIAN_JALV_FILTER'] = ''

        if errors:
            logging.error(
                "Configuring JALV LV2-Plugins  failed: {}".format(errors))
            self.clear()
            self.set_status(400)
            self.finish(
                "Configuring JALV LV2-Plugins failed: {}".format(errors))
    ...
    def do_enable_plugins(self):
        try:
            postedPlugins = tornado.escape.recursive_unicode(
