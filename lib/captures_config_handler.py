...
class CapturesConfigHandler(ZynthianBasicHandler):
    ...
    def do_rename(self):
        ...
        if self.get_argument('ZYNTHIAN_CAPTURES_RENAME'):
            newName = re.split(
                '[.](?![^][]*\])', self.get_argument('ZYNTHIAN_CAPTURES_RENAME'))[0]
        if self.get_argument('ZYNTHIAN_CAPTURES_NAME'):
            newNameExtension = re.split(
                '[.](?![^][]*\])', self.get_argument('ZYNTHIAN_CAPTURES_NAME'))[1]
    ...
    def install_file(self, fpath):
        ...
        destination = "{}/{}.{}".format(
            CapturesConfigHandler.CAPTURES_DIRECTORY, fname, fext[1:4])
