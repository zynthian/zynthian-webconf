...
class SecurityConfigHandler(ZynthianConfigHandler):
    ...
    def update_system_config(self, config):
        ...
        try:
            root_crypt = check_output(
                "getent shadow root", shell=True).decode("utf-8").split(':')[1]
            rcparts = root_crypt.split('$')
            current_crypt = crypt(current_passwd, "$%s$%s" %
                                  (rcparts[1], rcparts[2]))
        ...
        if len(config['PASSWORD'][0]) > 0:
            ...
            try:
                check_output(
                    ['usermod', '-p', crypt(config['PASSWORD'][0]), 'root'])
            ...
            try:
                self.update_hostapd_conf(
                    "wpa_passphrase", config['PASSWORD'][0])
            except Exception as e:
                logging.error(
